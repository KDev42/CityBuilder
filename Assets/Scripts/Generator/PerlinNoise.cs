...
public class PerlinNoise : MonoBehaviour
{
    ...
    public float[,] Generate(int sizeX, int sizeY, int octaves, float persistence )
    {
        float[,] heighMap = new float[sizeX, sizeY];

        for (int i = 0; i < sizeX; i++)
            for (int j = 0; j < sizeY; j++)
                heighMap[i, j] = Noise(i* 0.0007f+0.05f, j* 0.0007f+0.05f, octaves, persistence);
